...
const connectEureka = async () => {
    const instanceID = `${process.env.HOST || 'localhost'}:${process.env.APP_NAME || 'userService'}:${process.env.PORT || 3000}`
    const eurekaHost = `${process.env.SSL ? 'https' : 'http'}://${process.env.EUREKA_HOST || 'localhost:8761'}/eureka/apps/${process.env.APP_NAME || 'userService'}`;
...
const eurekaRegister = async (instanceID, eurekaHost) => {
    const dataSet = {
        instance: {
            ...
            app: `${process.env.APP_NAME || 'userService'}`,
            vipAddress: `${process.env.VIP_ADDRESS}`,
            ipAddr: `${process.env.IP_ADDR || 'jq.test.userService.com'}`,
