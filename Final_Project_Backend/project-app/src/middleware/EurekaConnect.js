...
const connectEureka = async () => {
    const instanceID = `${process.env.HOST || 'localhost'}:${process.env.APP_NAME || 'applicationServer'}:${process.env.PORT || 3001}`
...
const eurekaRegister = async (instanceID, eurekaHost) => {
    const dataSet = {
        instance: {
            hostName: `${process.env.HOST || 'localhost'}:${process.env.PORT || 3001}`,
            ...
            healthCheckUrl: `${process.env.SSL ? 'https' : 'http'}://${process.env.HOST || 'localhost'}:${process.env.PORT || 3001}/api/health/`,
            statusPageUrl: `${process.env.SSL ? 'https' : 'http'}://${process.env.HOST || 'localhost'}:${process.env.PORT || 3001}/api/status/`,
            homePageUrl: `${process.env.SSL ? 'https' : 'http'}://${process.env.HOST || 'localhost'}:${process.env.PORT || 3001}/api/`,
            dataCenterInfo: {
